import { ask, chain, doEffect, map, provideSome, zip } from '@typed/fp'
...
export const hashDirectory = (directory: string) =>
  doEffect(function* () {
    ...
    const dependencies: readonly Document[] = yield* pipe(
      supportedDocuments.map(readDependencies),
      zip,
      map((docs) => uniqDocumentPaths(docs.flat())),
      provideSome({ documentRegistry: supportedDocumentRegistry }),
    )
